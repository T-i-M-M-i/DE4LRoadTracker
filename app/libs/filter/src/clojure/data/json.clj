...
#_(defn- write-instant [^java.time.Instant x ^Appendable out options]
  (let [formatter ^java.time.format.DateTimeFormatter (:date-formatter options)]
    (.append out \")
    (.append out (.format formatter x))
    (.append out \")))

#_(defn- write-date [^java.util.Date x ^Appendable out options]
...
#_(defn- write-sql-date [^java.sql.Date x ^Appendable out options]
...
#_(extend java.time.Instant      JSONWriter {:-write write-instant})
#_(extend java.util.Date         JSONWriter {:-write write-date})
#_(extend java.sql.Date          JSONWriter {:-write write-sql-date})
